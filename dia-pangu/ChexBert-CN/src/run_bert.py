...
from datasets.ctrg_dataset import CTRGCheXbertDataset
...
def load_data(annotation_path, label_csv_path, batch_size=BATCH_SIZE,
     ...
     # 创建训练集 Dataset
     train_dset = CTRGCheXbertDataset(annotation_path, label_csv_path, split='train')
     # 创建验证集 Dataset
     dev_dset = CTRGCheXbertDataset(annotation_path, label_csv_path, split='val')


     train_loader = torch.utils.data.DataLoader(train_dset, batch_size=batch_size, shuffle=shuffle,
                                           num_workers=num_workers, collate_fn=collate_fn)
     dev_loader = torch.utils.data.DataLoader(dev_dset, batch_size=batch_size, shuffle=False,
                                         num_workers=num_workers, collate_fn=collate_fn)
...
def train(save_path, dataloaders, f1_weights, model=None, device=None,
     ...
     if model is None:
          ...
          # 移除或注释掉 DataParallel，它不适用于NPU，单卡训练无需并行
     ...
     for epoch in range(NUM_EPOCHS):
          for i, data in enumerate(train_ld, 0):
               ...
               if (i+1) % valid_niter == 0:
                    ...
                    print('average: %.3f' % (np.nanmean(kappas)))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train BERT-base model on CTRG-Chest dataset.')
    # 修改命令行参数
    parser.add_argument('--ann_path', type=str, nargs='?', required=True,
                        help='path to annotation.json file.')
    parser.add_argument('--label_path', type=str, nargs='?', required=True,
    ...
    # 注意: get_weighted_f1_weights 函数也需要修改以处理原始CSV
    f1_weights = utils.get_weighted_f1_weights(label_path)
    # 使用新的参数调用 load_data
    dataloaders = load_data(ann_path, label_path)
