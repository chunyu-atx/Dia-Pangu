...
# ===========================
# EN checkpoint defaults
# ===========================
DEFAULT_EN_CKPT = "/media/t1/shujie/dia-pangu/checkpoint_EN/P1-FLLM_en/pytorch_model.cpu_state_dict.bin"
SCRIPT_VERSION = "v0112_en"

# ====== LLM 层前缀/正则（与你给的英文 ckpt keys 一致） ======
...
def ensure_llm_prefix_exists(sd: Dict[str, torch.Tensor]) -> None:
    """
    防止用户传错 ckpt（比如只有 vision 或别的结构）。
    """
    hit = any(k.startswith(LLM_LAYER_PREFIX) for k in sd.keys())
    if not hit:
        # 给一点提示：找一些 layer 类前缀候选
        prefixes = {}
        for k in sd.keys():
        ...
        top = sorted(prefixes.items(), key=lambda x: x[1], reverse=True)[:10]
        hint = "\n".join([f"  {p} -> {c}" for p, c in top]) if top else "  (none)"
        raise RuntimeError(
            f"Cannot find expected LLM layer prefix: {LLM_LAYER_PREFIX}\n"
            f"Top layer-prefix candidates:\n{hint}"
        )
...
def clean_merged_llm_layers_inplace(sd: Dict[str, torch.Tensor], new_layers: int) -> None:
    ...
    否则会残留原 ckpt 的高层（例如 layer33 之类的问题）。
    """
...
class LLMOnlyPruneFriendly(torch.nn.Module):
    ...
    生成与 ckpt 对齐的 key 结构：
      ...
      且对 target_modules 生成 base_layer + lora_A/B（与你给的英文 ckpt key 一致）
    """
...
def main():
    parser = argparse.ArgumentParser(f"LLM-only prune + merge + clean ({SCRIPT_VERSION}) [EN defaults]")

    # EN 默认 ckpt：你提供的路径
    parser.add_argument(
        "--ckpt",
        type=str,
        default=DEFAULT_EN_CKPT,
        help=f"Original full checkpoint (default EN): {DEFAULT_EN_CKPT}",
    )

    # lang-model-path：可选，默认使用 ckpt 所在目录
    parser.add_argument(
        "--lang-model-path",
        type=str,
        default="",
        help="Base LLM model directory (default: ckpt dir). Must contain config.json etc.",
    )
    ...
    if not args.lang_model_path.strip():
        args.lang_model_path = os.path.dirname(args.ckpt)

    drop_layers = parse_layers_spec(args.drop_layers)
    tag = (args.tag.strip() or make_drop_tag(drop_layers))

    out_llm_only = os.path.join(args.out_dir, f"llm_only_pruned_state_{tag}_{SCRIPT_VERSION}_cpu.bin")
    out_merged_clean = os.path.join(args.out_dir, f"pytorch_model_pruned_{tag}_merged_clean_{SCRIPT_VERSION}_cpu.bin")
    out_map = os.path.join(args.out_dir, f"prune_layer_map_{tag}_{SCRIPT_VERSION}_cpu.json")
    ...
    print(f"[ARGS] version={SCRIPT_VERSION}")

    sd_orig = load_checkpoint_state_dict(args.ckpt)

    # 小检查：确保是目标结构（你的英文 keys 是匹配的）
    ensure_llm_prefix_exists(sd_orig)
