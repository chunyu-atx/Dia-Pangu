...
class MultiPanguForCausalLM(nn.Module):
    def __init__(self, text_tokenizer_path, lang_model_path, num_hidden_layers=None, device_map=None, enable_gradient_checkpointing=False):
        ...
        if num_hidden_layers is None:
         # 旧用法：保持兼容
            ...
            trust_remote_code=True,
            torch_dtype="auto",
            device_map=device_map,   # 这里别写死 "npu"，让外部控制
            local_files_only=True
            )
        else:
    # 新用法：构建剪枝后的层数（比如 28）
        ...
        self.lang_model = AutoModelForCausalLM.from_config(cfg, trust_remote_code=True)
