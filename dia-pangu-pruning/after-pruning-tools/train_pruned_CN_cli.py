...
# ------------------------ dist / device helpers ------------------------
def maybe_init_dist(enable_dist: bool):
    """
    单卡默认：enable_dist=False -> 永不 init dist
    多卡（torchrun）时：enable_dist=True 且环境变量齐全 -> init dist
    """
    if not enable_dist:
        return
...
def choose_device(npu_id: int) -> Tuple[torch.device, int]:
    """
    单卡 python 启动：用 npu_id
    torchrun 启动：优先用 LOCAL_RANK
    """
    local_rank_env = os.environ.get("LOCAL_RANK", None)
    use_id = int(local_rank_env) if local_rank_env is not None else int(npu_id)

    if torch.npu.is_available():
        torch.npu.set_device(use_id)
        return torch.device("npu", use_id), use_id
    if torch.cuda.is_available():
        torch.cuda.set_device(use_id)
        return torch.device("cuda", use_id), use_id
    return torch.device("cpu"), use_id


def warn_if_dist_env_present_but_disabled(enable_dist: bool):
    if enable_dist:
        return
    suspect_keys = ["RANK", "WORLD_SIZE", "LOCAL_RANK", "MASTER_ADDR", "MASTER_PORT"]
    present = {k: os.environ.get(k) for k in suspect_keys if k in os.environ}
    if present:
        print("[WARN] Detected distributed env vars but --enable_dist is NOT set; they will be ignored:", flush=True)
        for k, v in present.items():
            print(f"  - {k}={v}", flush=True)
        print("[WARN] If you intended multi-card, run with torchrun AND add --enable_dist.", flush=True)
...
class ModelArguments:
    ...
    # 剪枝后合并 clean 的权重
    pruned_merged_ckpt: str = field(default="")

    # 你可以直接写 "23-28" 或 "23,24,..."（若提供 prune_map 则可不填）
    drop_layers: str = field(default="")

    # 如果你给 prune_map，就从里面读 drop_layers（优先）
    prune_map: str = field(default="")

    # ✅ 单卡 python 启动时选择 npu 序号
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # ✅ 默认单卡：不启用分布式初始化
    enable_dist: bool = field(default=False)
...
def main():
    ...
    warn_if_dist_env_present_but_disabled(training_args.enable_dist)
    maybe_init_dist(training_args.enable_dist)
    set_seed(42)

    device, used_id = choose_device(model_args.npu_id)
    # 兼容 Trainer 里对 local_rank 的使用：没 torchrun 就固定 0
    local_rank = int(os.environ.get("LOCAL_RANK", 0)) if training_args.enable_dist else 0

    # ===== 确定 drop_layers（prune_map 优先）=====
    if model_args.prune_map.strip():
        pm = load_prune_map(model_args.prune_map)
        drop_layers_list = pm.get("drop_layers", [])
        if not isinstance(drop_layers_list, list):
            raise ValueError("prune_map.drop_layers must be a list")
    ...
    if not model_args.pruned_merged_ckpt.strip():
        raise ValueError("--pruned_merged_ckpt is required")
    ...
    if get_rank0() == 0:
        run_started = {
            ...
            "training_args": {
                ...
                "enable_dist": bool(training_args.enable_dist),
            },
        }
        safe_write_json(paths["run_started_json"], run_started)
        print(f"[env] rank={get_rank0()} local_rank={local_rank} device={device} (npu_id_used={used_id})", flush=True)
        print(f"[ckpt] pruned_merged_ckpt={model_args.pruned_merged_ckpt}", flush=True)
    ...
    ckpt_obj = torch.load(model_args.pruned_merged_ckpt, map_location="cpu")
    ckpt_sd = normalize_state_dict_obj(ckpt_obj)

    inferred_targets, inferred_r = infer_lora_meta_from_ckpt_v0112(ckpt_sd)
    adapter_cfg = load_adapter_config_if_exists(model_args.pruned_merged_ckpt)
    ...
    if get_rank0() == 0:
        safe_write_json(
            paths["lora_meta_json"],
            {
                ...
                "adapter_config_used": bool(adapter_used),
            },
        )
        print(
            f"[lora-meta] target_modules={target_modules}, r={lora_r}, alpha={lora_alpha}, adapter_config_used={adapter_used}",
            flush=True,
        )
    ...
    if get_rank0() == 0:
        ...
        print(
            f"[step] optimizer_steps_per_epoch≈ceil({micro_steps_per_epoch}/{training_args.gradient_accumulation_steps})={opt_steps_per_epoch}",
            flush=True,
        )
    ...
    try:
        ...
        if get_rank0() == 0:
            ...
            timing = {
                ...
                "avg_seconds_per_optimizer_step": (float(train_seconds) / float(gs))
                ...
                "ckpt_path": model_args.pruned_merged_ckpt,
                "meta_files": {
